...
int sem_func_decl(Pnode root, Phash_node f_loc_env, int not_first){
	...
	current = current->brother;
	int var_sect_opt_ok = sem_var_sect_opt(current, new_f_loc_env);
	current = current->brother;
	int const_sect_opt_ok = sem_const_sect_opt(current, new_f_loc_env);
	current = current->brother;
...
int sem_func_list_opt(Pnode root, Phash_node f_loc_env){
    ...
    while (func_decl_node != NULL) {
        ok = ok && sem_func_decl(func_decl_node, f_loc_env, 1);
...
int sem_stat(Pnode root, Phash_node f_loc_env, int * w_return){
    ...
    switch (child->value.ival) {//always a NONTERMINAL
        ...
        case NWHILE_STAT:
            ...
            break;
        case NFOR_STAT:
            ok = sem_for_stat(child, f_loc_env);
            break;
        case NFOREACH_STAT:
            ok = sem_foreach_stat(child, f_loc_env);
            break;
        case NRETURN_STAT:
        ...
        case NREAD_STAT:
            ...
            break;
        case NWRITE_STAT:
            ok = sem_write_stat(child, f_loc_env);
            break;
        default:
            printf("\n##%d\n", child->value.ival);
...
int sem_assign_stat(Pnode root, Phash_node f_loc_env){
    ...
    ok = ok && sem_left_hand_side(lhs_node, f_loc_env, &lhs_schema, &lhs_class);
    ...
    switch (child->type) {
        ...
        case T_NONTERMINAL:
            switch (child->value.ival) {
                case NFIELDING:
                    //printf("##field ");
                    lhs_ok = sem_fielding(child, f_loc_env, stype, lhs_class);
                    printSchema(*stype, " ");
                    break;
                case NINDEXING:
                    //printf("##index ");
                    lhs_ok = sem_indexing(child, f_loc_env, stype, lhs_class);
                    printSchema(*stype, " ");
                    break;
                default:
...
int sem_fielding(Pnode root, Phash_node f_loc_env, Pschema * stype, Class * lhs_class){
    ...
    ok_field = sem_left_hand_side(lhs_node, f_loc_env, &lhs_type, lhs_class);
...
int sem_indexing(Pnode root, Phash_node f_loc_env, Pschema * stype, Class * lhs_class){
    ...
    ok_index = ok_index && sem_expr(index_node, f_loc_env, &index_type);
...
int sem_wr_expr(Pnode root, Phash_node f_loc_env, Pschema * stype){
    ...
    int expr_ok = 1;
    if (ok) {
        expr_ok = sem_expr(root->child->brother, f_loc_env, stype);
    }
    return ok && expr_ok;
}
...
int sem_instance_expr(Pnode root, Phash_node f_loc_env, Pschema * stype){
	...
	switch(root->qualifier){
		case STRUCT:
			...
			expr_ok = sem_expr(current_node, f_loc_env, &current_schema); //eval first child's schema
			...
			while (current_node){//cicle for the other brother
				...
				expr_ok = sem_expr(current_node, f_loc_env, &next);
		...
		case VECTOR:
			...
			expr_ok = sem_expr(current_node, f_loc_env, &current_schema);
			current_node = current_node->brother;
			count++;
			while (current_node){
				Pschema next = new_schema_node(-1);
...
int sem_cond_expr(Pnode root, Phash_node f_loc_env, Pschema * stype){
	...
	int elsif_expr_ok = sem_elsif_expr_list_opt(elsif_expr_node, f_loc_env, &elsif_expr_type);
...
int sem_elsif_expr_list_opt(Pnode root, Phash_node f_loc_env, Pschema * stype){
    ...
    int expr_ok = sem_expr(expr_node, f_loc_env, expr_type);
		...
		case T_NONTERMINAL:
            switch(root->value.ival){
                ...
                case NLOGIC_EXPR:
                    ...
                    break;
                case NREL_EXPR:
                    expr_ok = sem_rel_expr(root, f_loc_env, stype);
                    break;
                case NNEG_EXPR:
                    expr_ok = sem_neg_expr(root, f_loc_env, stype);
                    break;
                case NWR_EXPR:
                    expr_ok = sem_wr_expr(root, f_loc_env, stype);
                    break;
                case NRD_EXPR:
                    expr_ok = sem_rd_expr(root, f_loc_env, stype);
                    break;
                case NINSTANCE_EXPR:
                    expr_ok = sem_instance_expr(root, f_loc_env, stype);
                    break;
                case NFUNC_CALL:
                    expr_ok = sem_func_call(root, f_loc_env, stype);
                    break;
                case NCOND_EXPR:
                    expr_ok = sem_cond_expr(root, f_loc_env, stype);
                    break;
                case NBUILT_IN_CALL:
                    expr_ok = sem_built_in_call(root, f_loc_env, stype);
                    break;
                default:
